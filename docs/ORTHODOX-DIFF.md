...
| Verify after copy: read the destination back and compare | DN, TC | Adopt | done in 4.24.0, as a checkbox on the copy form. Nothing in the tree hashes anything but sftp host keys, and this is what makes a copy to a failing stick or a remote target trustworthy |
| Create and verify `.sha256` files for the marked entries | TC | Adopt | done in 4.24.0. No md5: `md5sum` reads the ones that already exist, and nothing should be writing new ones |
| **Apply a command once per marked file** | Far (`C-g`) | Adopt | done in 4.23.0, as one script rather than one job, so the output is the terminal's. `[[commands]]` expands `%s` to every marked file in one invocation, which is a different tool |
| Wipe: overwrite, then unlink | Far (`A-Del`) | Adopt | done in 4.23.0, with the confirm saying what one pass is and is not worth on a copy-on-write filesystem |
...
| Restore the previous selection | Far (`C-m`) | Adopt | done in 4.22.0 on `C-x m`, Far's own key being Enter in a terminal (§10). Marks die to `%u`, to a reload and to every operation; one key that brings the last set back is a handful of lines against the panel's mark storage. Far's key is Enter in a terminal (§10) |
...
| Hide one panel outright | VC (`C-F1`/`C-F2`) | Adopt | done in 4.22.0, on VC's own keys |
| Directory sizes for every directory in the panel at once | TC | Adopt | done in 4.22.0 on `C-x space`; the scans are still one at a time |
...
1. ~~**Directory synchronize**~~ - shipped in 4.14.0. Half of it was
   ...
   the direction rule that makes a plan possible at all.
2. ~~**Archive creation, `A-F5`**~~ - shipped in 4.12.0. The writers
