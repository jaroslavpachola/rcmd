...
| Feature | From | Decision | Note |
|---|---|---|---|
| **Include-exclude masks** (`*.c,*.h\|*_test.*`) as one language everywhere | Far | **Adopt - first** | done in 4.11.0 for the select, unselect, filter and find dialogs; `[[highlight]]` and `[[open]]` still speak the single glob. Far's real trick is not the syntax, it is that select, filter, highlighting, associations and sort groups all speak it. `pattern.rs` has the glob and regex halves already; adding comma lists and the `\|` exclusion upgrades `+`, `-`, `C-f`, `[[highlight]]` and `[[open]]` in one change, and every row below leans on it |
