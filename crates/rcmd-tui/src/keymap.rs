...
const DEFAULTS: &[(&str, &str)] = &[
    ...
    ("alt+x", "palette"),      // every action by name, as Emacs's M-x
...
/// Every panel action by the name the config, the socket and the
/// command palette call it.
pub const ACTIONS: &[(&str, Action)] = &[
    ...
    ("sort-atime", Action::Sort(SortKey::Atime)),
    ("sort-ctime", Action::Sort(SortKey::Ctime)),
    ("sort-owner", Action::Sort(SortKey::Owner)),
